...
3. Phase 1 - First proof slice: enumerate inner ZIPs only, depends on step 2.
   Open the outer ZIP, list entries, filter to the expected HD_HANDLER_*.zip members, and log the names plus count. No CSV access yet. This is the first user-facing test milestone and should be usable against both local storage and S3 with identical behavior using the same script arguments pattern we keep throughout development.
4. Phase 1 - Add explicit schema and structure assertions for the archive layout, depends on step 3.
   Hard-fail if the expected inner ZIP inventory is missing, empty, duplicated unexpectedly, or named differently than assumed. This keeps later steps specific to the known input rather than defensive for many variants.
5. Phase 2 - Second proof slice: open one inner ZIP and enumerate its contents, depends on step 4.
   Open the first expected inner ZIP from inside the outer ZIP without extraction, list its members, and assert the expected CSV structure once observed. This isolates nested ZIP handling before introducing pandas.
6. Phase 2 - Third proof slice: read CSV header only from one inner ZIP, depends on step 5.
   Open the inner CSV as a stream and read just enough to capture the header row and column names. Log the column count and selected field names required by the sieve: CURRENT RECORD, INCLUDE IN NATIONAL REPORT, TSD ACTIVITY, FED WASTE GENERATOR, LOCATION LONGITUDE, LOCATION LATITUDE. Hard-fail if any required columns are missing.
7. Phase 2 - Fourth proof slice: chunk-read one inner CSV without filtering, depends on step 6.
   Use pandas.read_csv(..., chunksize=...) on the streamed inner CSV and log chunk counts and row totals for one file only. This proves the memory model and installed dependencies before any filtering logic is introduced.
...
9. Phase 3 - Fifth proof slice: filter one inner CSV and report counts only, depends on step 8.
   Run the chunked sieve over one inner CSV, accumulate only per-chunk and total survivor counts, and log the yield. This gives a clean checkpoint that the filter logic matches expectations before file output is added.
10. Phase 3 - Sixth proof slice: write filtered rows from one inner CSV to a local CSV output, depends on step 9.
...
11. Phase 3 - Seventh proof slice: write filtered rows from one inner CSV to AWS output, depends on step 10.
    Using the same one-inner-CSV processing logic, add S3 output writing and validate that remote output works cleanly. Treat this as its own milestone because AWS output is a distinct technical risk. If append semantics are awkward on S3, choose the simplest implementation that still preserves correctness for one-inner-CSV testing and document that choice in the code and plan.
12. Phase 4 - Generalize from one inner ZIP to all inner ZIPs into one consolidated output, depends on step 11.
    Loop over all enumerated inner ZIPs, stream/filter each CSV in turn, append to the same output target, and maintain total input and survivor counts. Explicitly close inner streams and ZIP handles between iterations. Because one-inner-CSV local and AWS paths are already proven, this step should mostly be expansion rather than architectural change.
...
**Relevant files**
- c:\openSource\dataPreservation\EJSCREEN-Data-Processing\scripts\hazardous_waste\hazardous_waste_preprocess.py — new script to build in incremental slices; currently empty.
- c:\openSource\dataPreservation\EJSCREEN-Data-Processing\scripts\hazardous_waste\processingSieveDesignPromptForCopilot.md — source of the staged streaming architecture, especially nested ZIP handling, chunked iteration, and single consolidated output.
...
**Verification**
1. After step 3, run the script against one local root and one s3:// root and confirm the same inner ZIP names and counts are logged.
2. After step 5, confirm the listed members from one inner ZIP match the actual archive contents.
...
8. After step 12, confirm total output row count equals the sum of survivors from all inner ZIP runs and that only one consolidated CSV is produced.

**Decisions**
- The first milestone is enumeration only: log inner ZIP names and counts, with no persisted inventory file.
...
- Move AWS output earlier: prove one-inner-CSV local output first, then one-inner-CSV AWS output, then generalize to all inner ZIPs.
...
1. If the real archive contains more than one CSV per inner ZIP, lock the expected filename pattern during step 5 before any filtering code is written.
2. For the one-inner-CSV AWS write milestone, decide implementation based on actual S3 append constraints: either a simple one-shot write of accumulated surviving rows for that single inner CSV or a streaming-compatible approach if testing shows it is straightforward.
