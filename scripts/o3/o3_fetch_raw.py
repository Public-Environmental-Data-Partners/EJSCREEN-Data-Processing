...
Purpose:
	Download the national 2020 O3 source .txt.gz file into the configured
	Ozone pipeline storage location.

Process summary:
	- Read Ozone configuration and choose local or remote storage mode.
...
Outputs:
	- downloads/2020/2020_o3_daily_average.txt.gz under the active Ozone root.
	- fetch_audit.csv under the active Ozone root.
	- o3_fetch_raw.log in scripts/o3.
"""
...
PROCESS_NAME = 'o3_fetch_raw'
DEFAULT_LOG_FILENAME = 'o3_fetch_raw.log'
...
def get_config(argv=None) -> Config:
	"""Parse the runtime arguments and merge them with validated Ozone config values."""
	raw_config = get_o3_config()

	parser = argparse.ArgumentParser(description='Download the raw national Ozone.txt.gz file to local or remote Ozone pipeline storage.')
	parser.add_argument(
		'storage_mode',
...
def download_to_temp_file(session: requests.Session, cfg: Config) -> tuple[Path, int]:
	"""Stream the configured Ozone source file into a temporary local file."""
...
def download_raw_o3_file(session: requests.Session, cfg: Config, destination_path: str, filename: str) -> DownloadResult:
	"""Download the raw Ozone file unless the configured destination already exists."""
	...
	logging.info('Downloading raw Ozone file from %s', cfg.source_url)
...
def main(argv=None) -> int:
	"""Run the Ozone raw-download workflow and append one audit record."""
	...
	with requests.Session() as session:
		...
		try:
			result = download_raw_o3_file(session, cfg, destination_path, filename)
	...
	logging.info('Completed raw Ozone fetch with status %s', result.status)
	return 0
