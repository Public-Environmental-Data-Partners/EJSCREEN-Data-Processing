...
one annual average concentration per tract:

   ```bash
   python3 pm25/pm25_preprocess.py -v 1.2020 -l local
   ```
...
3. `pm25_score.py` expands tract scores to block groups, applies the zero-population null rule, and writes per-state final scores:
   ...
   ```bash
   python3 pm25/pm25_score.py -v 1.2020 -s WY -l local 
...
1. Pull an EJAM PM2.5 subset for a state (to folder under `pipeline/compare/ejamOG/{STATE}/` for `--location local`):
   ...
   ```bash
   python3 utilities/validation/ejam2csv.py --state WY --location local --indicator pm25  --location local
   ```
